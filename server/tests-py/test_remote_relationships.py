...
class TestCreateRemoteRelationship:
    ...
    def test_generation(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_basic.yaml')
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_nested_args.yaml')

        check_query_f(hge_ctx, f'{self.dir()}select_remote_fields.yaml')
...
class TestDeleteRemoteRelationship:
    ...
    def test_delete(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_basic.yaml')
        hge_ctx.v1q_f(f'{self.dir()}delete_remote_rel.yaml')

    def test_delete_dependencies(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_basic.yaml')
        hge_ctx.v1q_f(
            f'{self.dir()}remove_remote_schema.yaml', expected_status_code=400
        )
        hge_ctx.v1q_f(f'{self.dir()}delete_remote_rel.yaml')

    def test_deleting_column_with_remote_relationship_dependency(self, hge_ctx):
        check_query_f(hge_ctx, f'{self.dir()}drop_col_with_remote_rel_dependency.yaml')
    ...
    def test_deleting_table_with_remote_relationship_dependency(self, hge_ctx):
        check_query_f(
            hge_ctx, f'{self.dir()}drop_table_with_remote_rel_dependency.yaml'
        )
        self._check_no_remote_relationships(hge_ctx, 'profiles')
...
class TestUpdateRemoteRelationship:
    ...
    def test_update(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_basic.yaml')
        check_query_f(hge_ctx, f'{self.dir()}basic_relationship.yaml')
        hge_ctx.v1q_f(f'{self.dir()}update_remote_rel_basic.yaml')
        check_query_f(hge_ctx, f'{self.dir()}update_basic_query.yaml')
...
class TestExecution:
    ...
    def test_basic_relationship_on_object(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_basic.yaml')
        check_query_f(hge_ctx, f'{self.dir()}query_with_object_rel.yaml')

        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_nested_args.yaml')
        check_query_f(hge_ctx, f'{self.dir()}query_with_arr_rel.yaml')

    def test_regression_7172(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_regression_7172.yaml')
        check_query_f(hge_ctx, f'{self.dir()}regression_7172.yaml')

    def test_basic_relationship_joining_singleton_to_array(self, hge_ctx):
        hge_ctx.v1q_f(
            f'{self.dir()}setup_remote_rel_joining_singleton_with_array.yaml'
        )
        check_query_f(
            hge_ctx,
            f'{self.dir()}basic_relationship_joining_singleton_with_array.yaml',
        )

    def test_basic_array(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_array.yaml')
        check_query_f(hge_ctx, f'{self.dir()}basic_array.yaml')

    def test_basic_array_without_join_key(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_array.yaml')
        check_query_f(hge_ctx, f'{self.dir()}basic_array_without_join_key.yaml')

    def test_multiple_fields(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_multiple_fields.yaml')
        check_query_f(hge_ctx, f'{self.dir()}basic_multiple_fields.yaml')

    # https://github.com/hasura/graphql-engine/issues/5448
    def test_remote_join_fields_with_null_joining_fields(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_null_joining_fields.yaml')
        check_query_f(hge_ctx, f'{self.dir()}remote_rel_with_null_joining_fields.yaml')

    def test_nested_fields(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_nested_fields.yaml')
        check_query_f(hge_ctx, f'{self.dir()}basic_nested_fields.yaml')

    def test_arguments(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_nested_args.yaml')
        check_query_f(hge_ctx, f'{self.dir()}query_with_arguments.yaml')
   ...
   #    check_query_f(hge_ctx, self.dir() + 'mixed_variables.yaml')  -- uses heterogenous execution, due to which this assert fails
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_nested_args.yaml')
        check_query_f(hge_ctx, f'{self.dir()}remote_rel_variables.yaml')

    def test_with_fragments(self, hge_ctx):
        check_query_f(hge_ctx, f'{self.dir()}mixed_fragments.yaml')
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_basic.yaml')
        check_query_f(hge_ctx, f'{self.dir()}remote_rel_fragments.yaml')

    def test_with_interface(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_with_interface.yaml')
        check_query_f(hge_ctx, f'{self.dir()}mixed_interface.yaml')
        check_query_f(hge_ctx, f'{self.dir()}remote_rel_interface.yaml')

    def test_with_union(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_with_union.yaml')
        check_query_f(hge_ctx, f'{self.dir()}remote_rel_union.yaml')

    def test_with_enum(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_with_enum.yaml')
        check_query_f(hge_ctx, f'{self.dir()}remote_rel_enum.yaml')

    def test_with_errors(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_basic.yaml')
        check_query_f(hge_ctx, f'{self.dir()}query_with_errors_obj.yaml')
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_nested_args.yaml')
        check_query_f(hge_ctx, f'{self.dir()}query_with_errors_arr.yaml')
    ...
    def test_with_aliased_remote_join_keys(self, hge_ctx):
        ...
        """
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_basic.yaml')
        check_query_f(hge_ctx, f'{self.dir()}basic_relationship_alias.yaml')

    def test_with_scalar_relationship(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_scalar.yaml')
        check_query_f(hge_ctx, f'{self.dir()}query_with_scalar_rel.yaml')

    def test_renaming_column_with_remote_relationship_dependency(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_nested_args.yaml')
        check_query_f(
            hge_ctx, f'{self.dir()}rename_col_with_remote_rel_dependency.yaml'
        )

    def test_renaming_table_with_remote_relationship_dependency(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_nested_args.yaml')
        check_query_f(
            hge_ctx, f'{self.dir()}rename_table_with_remote_rel_dependency.yaml'
        )
    ...
    def test_remote_joins_in_mutation_response(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_basic_with_authors.yaml')
        check_query_f(
            hge_ctx, f'{self.dir()}mutation_output_with_remote_join_fields.yaml'
        )
...
class TestDeepExecution:
    ...
    def transact(self, hge_ctx, graphql_service):
        ...
        hge_ctx.v1q_f(f'{self.dir()}setup_address.yaml')
        yield
        hge_ctx.v1q_f(f'{self.dir()}teardown_address.yaml')
        hge_ctx.v1q_f(f'{self.dir()}teardown.yaml')

    def test_with_deep_object(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_basic.yaml')
        check_query_f(hge_ctx, f'{self.dir()}query_with_deep_nesting_obj.yaml')

    def test_with_deep_array(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_nested_args.yaml')
        check_query_f(hge_ctx, f'{self.dir()}query_with_deep_nesting_arr.yaml')

    def test_with_complex_path_object(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_basic.yaml')
        check_query_f(
            hge_ctx, f'{self.dir()}query_with_deep_nesting_complex_path_obj.yaml'
        )
        check_query_f(
            hge_ctx, f'{self.dir()}query_with_deep_nesting_complex_path_obj2.yaml'
        )

    def test_with_complex_path_array(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_basic.yaml')
        check_query_f(
            hge_ctx, f'{self.dir()}query_with_deep_nesting_complex_path_arr.yaml'
        )
        check_query_f(
            hge_ctx, f'{self.dir()}query_with_deep_nesting_complex_path_arr2.yaml'
        )
...
class TestExecutionWithPermissions:
    ...
    def transact(self, hge_ctx, graphql_service):
        print("In setup method")
        hge_ctx.v1q_f(f'{self.dir()}setup_with_permissions.yaml')
        yield
        hge_ctx.v1q_f(f'{self.dir()}teardown.yaml')
    ...
    def test_basic_relationship(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_basic.yaml')
        check_query_f(
            hge_ctx, f'{self.dir()}basic_relationship_with_permissions1.yaml'
        )
        check_query_f(
            hge_ctx, f'{self.dir()}basic_relationship_with_permissions2.yaml'
        )
    ...
    def test_complex_multiple_joins(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_multiple_remote_rel.yaml')
        check_query_f(hge_ctx, f'{self.dir()}complex_multiple_joins.yaml')
...
class TestWithRelay:
    ...
    def test_with_relay_fail(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_basic.yaml')
        check_query_f(hge_ctx, f"{self.dir()}with_relay.yaml")
...
class TestComputedFieldsInRemoteRelationship:
    ...
    def transact(self, hge_ctx, graphql_service):
        print("In setup method")
        hge_ctx.v1q_f(f'{self.dir()}setup.yaml')
        hge_ctx.v1q_f(f'{self.dir()}setup_remote_rel_computed_fields.yaml')
        yield
        hge_ctx.v1q_f(f'{self.dir()}teardown.yaml')

    def test_remote_join_with_computed_field(self, hge_ctx):
        check_query_f(hge_ctx, f'{self.dir()}remote_join_with_computed_field.yaml')

    def test_remote_join_with_computed_field_session(self, hge_ctx):
        check_query_f(
            hge_ctx, f'{self.dir()}remote_join_with_computed_field_session.yaml'
        )
...
class TestRemoteRelationshipFieldType:
    ...
    def test_remote_relationship_field_type(self, hge_ctx):
        hge_ctx.v1q_f(f'{self.dir()}/setup_remote_rel_nested_args.yaml')
        check_query_f(hge_ctx, f'{self.dir()}/remote_relationship_field_type.yaml')
