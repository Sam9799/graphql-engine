...
class TestQueryCache:
    ...
    def test_simple_cached_endpoint(self, hge_ctx, transport):
        check_query_f(hge_ctx, f'{self.dir()}/simple_cached.yaml', transport)
        self.flushRedis()

    def test_metrics_one_endpoint(self, hge_ctx, transport):
        check_query_f(hge_ctx, f'{self.dir()}/test_metrics_one.yaml', transport)
        self.flushRedis()

    def test_metrics_two_endpoint(self, hge_ctx, transport):
        check_query_f(hge_ctx, f'{self.dir()}/test_metrics_two.yaml', transport)
        self.flushRedis()

    def test_cache_clear_endpoint(self, hge_ctx, transport):
        check_query_f(hge_ctx, f'{self.dir()}/test_cache_clear.yaml', transport)
        self.flushRedis()

    def test_cache_clear_endpoint_key(self, hge_ctx, transport):
        check_query_f(hge_ctx, f'{self.dir()}/test_cache_clear_key.yaml', transport)
        self.flushRedis()

    def test_cache_clear_endpoint_family(self, hge_ctx, transport):
        check_query_f(hge_ctx, f'{self.dir()}/test_cache_clear_family.yaml', transport)
        self.flushRedis()

    def test_no_variables_in_query_key(self, hge_ctx, transport):
        check_query_f(
            hge_ctx, f'{self.dir()}/test_no_variables_in_query_key.yaml', transport
        )
        self.flushRedis()
    ...
    def test_action_query_with_forward_client_headers_set(self, hge_ctx, transport):
        check_query_f(
            hge_ctx,
            f'{self.dir()}/test_action_query_with_forward_client_headers_set.yaml',
            transport,
        )
        self.flushRedis()
    ...
    def test_action_query_with_forward_client_header_unset(self, hge_ctx, transport):
        check_query_f(
            hge_ctx,
            f'{self.dir()}/test_action_query_with_forward_client_headers_unset.yaml',
            transport,
        )
        self.flushRedis()
